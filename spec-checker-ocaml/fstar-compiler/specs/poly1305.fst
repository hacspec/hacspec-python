...
let subblock_t: Type0 = (x: vlarray_t uint8_t {(bytes_length x) <=. 16})
let p130m5: nat = (2 **. 130) -. 5
let felem_t: Type0 = natmod_t p130m5
let felem (n: nat) : natmod_t p130m5 = natmod n p130m5
let encode (block: (x: vlarray_t uint8_t {(bytes_length x) <=. 16})) : natmod_t p130m5 =
  ...
  lelem +. welem
let encode_r (r: array_t uint8_t 16) : natmod_t p130m5 =
  ...
  felem r_nat
let poly (text: vlarray_t uint8_t) (r: natmod_t p130m5) : natmod_t p130m5 =
  let blocks, last = array_split_blocks text blocksize in
  let acc = felem 0 in
  let acc =
    repeati (array_length blocks)
      (fun i acc ->
          let acc = (acc +. (encode blocks.[ i ])) *. r in
          acc)
      acc
  in
  let acc =
    ...
    then
      (let acc = (acc +. (encode last)) *. r in
        acc)
    else acc
  in
  acc
let poly1305_mac (text: vlarray_t uint8_t) (k: array_t uint8_t 32) : array_t uint8_t 16 =
