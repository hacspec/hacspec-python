...
let sha2 (v:variant) : None =
  ...
  let maj (x:word_t) (y:word_t) (z:word_t) : word_t =
    ((x &. y) ^. ((x &. z) ^. (y &. z))) in 
  let sigma (x:word_t) (i:range_t 0x0 0x4) (op:range_t 0x0 0x1) : word_t =
    let tmp = if ((op = 0x0)) then (let tmp = (x >>. opTable.[((0x3 *. i) +. 0x2)]) in tmp )else (let tmp = word_t.rotate_right x opTable.[((0x3 *. i) +. 0x2)] in tmp) in 
  ...
  let schedule (block:block_t) : k_t =
    ...
    let s = create kSize (word_t 0x0) in 
    let s = repeati (range 0x10)
      (fun i s ->
        let s = s.[i] <- b.[i] in 
        s)
      s in 
    let (s,t15,t7,t2,s0,t16,s1) = repeati (range 0x10 kSize)
      (fun i (s,t15,t7,t2,s0,t16,s1) ->
        let t16 = s.[(i -. 0x10)] in 
        let t15 = s.[(i -. 0xf)] in 
        let t7 = s.[(i -. 0x7)] in 
        let t2 = s.[(i -. 0x2)] in 
        ...
        (s,t15,t7,t2,s0,t16,s1))
      (s,t15,t7,t2,s0,t16,s1) in 
    s in 
  let shuffle (ws:k_t) (hashi:hash_t) : hash_t =
    let h = copy hashi in 
    let (g0,h,c0,a0,t1,e0,t2,d0,b0,h0,f0) = repeati (range kSize)
      (fun i (g0,h,c0,a0,t1,e0,t2,d0,b0,h0,f0) ->
        let a0 = h.[0x0] in 
        let b0 = h.[0x1] in 
        let c0 = h.[0x2] in 
        let d0 = h.[0x3] in 
        ...
        let h = h.[0x6] <- f0 in 
        let h = h.[0x7] <- g0 in 
        (g0,h,c0,a0,t1,e0,t2,d0,b0,h0,f0))
      (g0,h,c0,a0,t1,e0,t2,d0,b0,h0,f0) in 
    h in 
  ...
  let hash (msg:vlbytes_t) : digest_t =
    ...
    let h = h0 in 
    let h = repeati (range nblocks)
      (fun i h ->
        let h = compress blocks.[i] h in 
        h)
      h in 
    let last_len = array.length last in 
    let len_bits = (array.length msg *. 0x8) in 
    let pad = create (0x2 *. blockSize) (u8 0x0) in 
    let pad = update_slice pad 0x0 last_len last in 
    let pad = pad.[last_len] <- u8 0x80 in 
    let (h,pad) = if ((last_len < (blockSize -. lenSize))) then (let pad = update_slice pad (blockSize -. lenSize) blockSize (len_to_bytes (len_t len_bits)) in 
      let h = compress slice pad 0x0 blockSize h in (h,pad) )else (let pad = update_slice pad ((0x2 *. blockSize) -. lenSize) (0x2 *. blockSize) (len_to_bytes (len_t len_bits)) in 
      let h = compress slice pad 0x0 blockSize h in 
      let h = compress slice pad blockSize (0x2 *. blockSize) h in (h,pad)) in 
    let result = words_to_bytes h in 
    truncate result in 
  hash 
let sha224 = sha2 0xe0 
let sha256 = sha2 0x100 
let sha384 = sha2 0x180 
let sha512 = sha2 0x200 
