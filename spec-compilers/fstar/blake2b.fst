...
open Speclib
let bits_in_word = 0x40 
let rounds_in_f = 0xc 
let block_bytes = 0x80 
let R1 = 0x20 
let R2 = 0x18 
let R3 = 0x10 
let R4 = 0x3f 
let working_vector_t = array_t uint64_t 0x10 
let hash_vector_t = array_t uint64_t 0x8 
...
let _G (v:working_vector_t) (a:index_t) (b:index_t) (c:index_t) (d:index_t) (x:uint64_t) (y:uint64_t) : working_vector_t =
  ...
  let v = v.[d] <- uint64_t.rotate_right (v.[d] ^. v.[a]) R1 in 
  let v = v.[c] <- (v.[c] +. v.[d]) in 
  let v = v.[b] <- uint64_t.rotate_right (v.[b] ^. v.[c]) R2 in 
  let v = v.[a] <- ((v.[a] +. v.[b]) +. y) in 
  let v = v.[d] <- uint64_t.rotate_right (v.[d] ^. v.[a]) R3 in 
  let v = v.[c] <- (v.[c] +. v.[d]) in 
  let v = v.[b] <- uint64_t.rotate_right (v.[b] ^. v.[c]) R4 in 
  v 
let _F (h:hash_vector_t) (m:working_vector_t) (t:uint128_t) (flag:bool) : hash_vector_t =
  let v = create 0x10 (u64 0x0) in 
  let v = update_slice v 0x0 0x8 h in 
  let v = update_slice v 0x8 0x10 IV in 
  let v = v.[0xc] <- (v.[0xc] ^. u64 t) in 
  let v = v.[0xd] <- (v.[0xd] ^. u64 (t >>. 0x40)) in 
  let v = if ((flag = True)) then (let v = v.[0xe] <- (v.[0xe] ^. u64 0xffffffffffffffff) in v )else (v) in 
...
let blake2b_internal (data:data_internal_t) (input_bytes:uint128_t) (kk:key_size_t) (nn:out_size_t) : contract vlbytes_t Lambda(args=arguments(args=arg(arg='data',
annotation=None,
type_comment=None)
arg(arg='input_bytes',
annotation=None,
...
body=((length res) = nn)) =
  let h = copy IV in 
  let h = h.[0x0] <- (((h.[0x0] ^. u64 0x1010000) ^. (u64 kk <<. 0x8)) ^. u64 nn) in 
  let data_blocks = (length data /. block_bytes) in 
  let () = if ((data_blocks > 0x1)) then (let () = repeati (range (data_blocks -. 0x1))
      (fun i () ->
        let h = _F h (vlbytes.to_uint64s_le slice data (block_bytes *. i) (block_bytes *. (i +. 0x1))) (u128 ((i +. 0x1) *. block_bytes)) False in 
        ())
      () in () )else (()) in 
  let () = if ((kk = 0x0)) then (let h = _F h (vlbytes.to_uint64s_le slice data (block_bytes *. (data_blocks -. 0x1)) (block_bytes *. data_blocks)) (u128 input_bytes) True in () )else (let h = _F h (vlbytes.to_uint64s_le slice data (block_bytes *. (data_blocks -. 0x1)) (block_bytes *. data_blocks)) (u128 (input_bytes +. block_bytes)) True in ()) in 
  slice vlbytes.from_uint64s_le h None nn 
let max_size_t = ((0x2 **. 0x40) -. 0x1) 
type data_t = x:vlbytes_t{(vlbytes.lenght x < (max_size_t -. (0x2 *. block_bytes)))}

let blake2b (data:data_t) (key:key_t) (nn:out_size_t) : contract vlbytes_t Lambda(args=arguments(args=arg(arg='data',
annotation=None,
type_comment=None)
arg(arg='key',
annotation=None,
...
body=((length res) = nn)) =
  ...
  blake2b_internal padded_data ll kk nn 
