...
let highbits_64 (x:uint64_t) : None =
  u32 (x >>. 0x20) 
let out_size_t = x:nat{(x <= 0x20)} 
let blake2 (v:variant) : None =
  let (hash_vector_t,data_internal_t,minus_one,_R4,word_t,_R3,data_t,to_double_word,working_vector_t,max_size_t,_R1,to_words_le,high_bits,block_bytes,from_words_le,key_size_t,low_bits,bits_in_word,index_t,rounds_in_f,_IV,key_t,to_word,_R2) = 
  if ((v = 0x1)) 
  then (let bits_in_word = 0x40 in 
    ...
    let data_internal_t = x:bytes{(length x < (0x2 **. 0x40)) && ((length x %. block_bytes) = 0x0)} in 
    let key_t = x:vlbytes{(length x <= 0x40)} in 
    ...
    let data_t = x:vlbytes{(length x < (max_size_t -. (0x2 *. block_bytes)))} in 
    (hash_vector_t,data_internal_t,minus_one,_R4,word_t,_R3,data_t,to_double_word,working_vector_t,max_size_t,_R1,to_words_le,high_bits,block_bytes,from_words_le,key_size_t,low_bits,bits_in_word,index_t,rounds_in_f,_IV,key_t,to_word,_R2) )
    ...
    let data_internal_t = x:bytes{(length x < (0x2 **. 0x40)) && ((length x %. block_bytes) = 0x0)} in 
    let key_t = x:vlbytes{(length x <= 0x20)} in 
    ...
    let data_t = x:vlbytes{(length x < (max_size_t -. (0x2 *. block_bytes)))} in 
    (hash_vector_t,data_internal_t,minus_one,_R4,word_t,_R3,data_t,to_double_word,working_vector_t,max_size_t,_R1,to_words_le,high_bits,block_bytes,from_words_le,key_size_t,low_bits,bits_in_word,index_t,rounds_in_f,_IV,key_t,to_word,_R2) ) in 
  ...
  let __F (h:hash_vector_t) (m:working_vector_t) (t:uint128_t) (flag:bool) : hash_vector_t =
    ...
    let (s,v) = repeati (range rounds_in_f)
      (fun i (s,v) ->
  ...
  blake2 
let blake2s = blake2 (variant (nat 0x0)) 
let blake2b = blake2 (variant (nat 0x1)) 
