...
let felem_t = assert_norm(p130m5 > 0); pfelem_t p130m5
let felem = assert_norm(p130m5 > 0); pfelem p130m5
let felem_to_int (x:felem_t) : nat = x 
let encode (block:subblock_t) : felem_t =
  let b = create 0x10 (u8 0x0) in 
  let b = update_slice b 0x0 (length block) (to_lbytes block) in 
  let welem = felem (uint_to_nat #U128 (uint_from_bytes_le #U128 b)) in 
  let lelem = felem (0x2 **. (0x8 `op_Multiply` (length block))) in 
  pfadd lelem welem
let encode_r (r:block_t) : felem_t =
  let ruint = uint_from_bytes_le #U128 r in 
  let ruint = u128 (ruint &. (u128 0xffffffc0ffffffc0ffffffc0fffffff)) in 
...
let poly (text:vlbytes_t) (r:felem_t) : felem_t =
  ...
  let acc = repeati (range (length blocks)) (fun i acc -> let acc = ((acc +. encode blocks.[i]) *. r) in  acc) acc in 
  let acc = if ((length last > 0x0)) then (let acc = ((acc +. encode last) *. r) in acc ) else (acc) in 
  acc 
let poly1305_mac (text:vlbytes_t) (k:key_t) : tag_t =
  ...
  let n = u128 ((u128 (felem_to_int a)) +. selem) in 
