...
let poly (text:vlbytes_t) (r:felem_t) : felem_t =
  ...
  let acc = repeati (range (length blocks)) (fun i acc -> let acc = fmul (fadd acc (encode blocks.[i])) r in  acc) acc in 
