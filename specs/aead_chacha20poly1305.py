...
@typechecked
def padded_aad_msg(aad:vlbytes,msg:vlbytes) -> Tuple[int,vlbytes]:
    laad = len(aad)
    lmsg = len(msg)
...
@typechecked
def aead_chacha20poly1305_encrypt(key:key_t,nonce:nonce_t,aad:vlbytes,msg:vlbytes) -> Tuple[vlbytes,tag_t]:
