...
def blocks(x:size_nat, m:size_nat) \
    -> contract(size_nat,
...
def mgf_sha256(mgfseed:vlbytes_t, maskLen:size_nat) \
    -> contract(vlbytes_t,
...
def pss_encode(salt:vlbytes_t, msg:vlbytes_t, emBits:size_nat) \
    -> contract(vlbytes_t,
                lambda salt, msg, emBits: array.length(msg) < max_input_len_sha256 and
                hLen + array.length(salt) + 2 <= blocks(emBits, 8) and
                array.length(salt) + hLen + 8 < max_size_t and emBits > 0,
                lambda salt, msg, emBits, res: array.length(res) == blocks(emBits, 8)):
    sLen = array.length(salt)
    emLen = blocks(emBits, 8)
    msBits = emBits % 8
...
def pss_verify(sLen:size_nat, msg:vlbytes_t, em:vlbytes_t, emBits:size_nat) \
    -> contract(bool,
                lambda sLen, msg, em, emBits: array.length(msg) < max_input_len_sha256 and
                array.length(em) == blocks(emBits, 8) and emBits > 0 and
...
def rsapss_sign(modBits:size_nat, skey:rsa_privkey, salt:vlbytes_t, msg:vlbytes_t) \
    -> contract(vlbytes_t,
                lambda modBits, skey, salt, msg: array.length(msg) < max_input_len_sha256 and modBits > 1 and
                array.length(salt) + hLen + 8 < max_size_t and array.length(salt) + hLen + 2 <= blocks(modBits - 1, 8),
                lambda modBits, skey, salt, msg, res: array.length(res) == blocks(modBits, 8)):
    (pkey, d) = skey
    (n, e) = pkey
    em = pss_encode(salt, msg, modBits - 1)
    m = os2ip(em)
    s = pow(m, d, n)
    sgnt = i2osp(s)
    return sgnt

def rsapss_verify(modBits:size_nat, pkey:rsa_pubkey, sLen:size_nat, msg:vlbytes_t, sgnt: vlbytes_t) \
    -> contract(bool,
    ...
    if s < n:
        m = pow(s, e, n)
        em = i2osp(m)
        res = pss_verify(sLen, msg, em, modBits - 1)
    else:
        res = False
    return res
