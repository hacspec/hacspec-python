...
def blake2(v:variant):
    if v == 1:
        ...
        low_bits = to_word
        high_bits = lambda x: to_word(x >> 64)
        ...
        low_bits = to_word
        high_bits = lambda x: to_word(x >> 32)
    ...
    def G(v: working_vector_t, a: index_t, b: index_t, c: index_t, d: index_t, x: uint64_t, y: uint64_t) -> working_vector_t:
        v[a] = v[a] + v[b] + x
        v[d] = word_t.rotate_right(v[d] ^ v[a], R1)
        v[c] = v[c] + v[d]
        v[b] = word_t.rotate_right(v[b] ^ v[c], R2)
        v[a] = v[a] + v[b] + y
        v[d] = word_t.rotate_right(v[d] ^ v[a], R3)
        v[c] = v[c] + v[d]
        v[b] = word_t.rotate_right(v[b] ^ v[c], R4)
        return v
    ...
    def blake2_internal(data: data_internal_t, input_bytes: uint128_t, kk: key_size_t, nn: out_size_t) \
        ...
        if kk == 0:
            h = F(h, to_words_le(
                data[block_bytes * (data_blocks - 1):block_bytes * data_blocks]), to_double_word(input_bytes), True)
        else:
            h = F(h, to_words_le(
                data[block_bytes * (data_blocks - 1):block_bytes * data_blocks]), to_double_word(input_bytes + block_bytes), True)
...
blake2s = blake2(variant(0))
blake2b = blake2(variant(1))
