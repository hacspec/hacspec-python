...
key_t = bytes_t(32)
tag_t = bytes_t(16)
subblock_t = refine(vlbytes_t, lambda x: bytes.length(x) <= 16)
subblock = subblock_t
...
def encode(block: subblock_t) -> felem_t:
    ...
    welem = to_felem(uintn.to_nat(bytes.to_uintn_le(b)))
