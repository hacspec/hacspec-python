...
word_t = uint32_t
kSize : int = 64
k_t = array_t(word_t,64)
opTableType_t = array_t(int,12)
opTable : opTableType_t = array([
    2, 13, 22,
    6, 11, 25,
    7, 18, 3,
    17, 19, 10])
...
def sha256(msg:vlbytes_t) -> digest_t:
    ...
    if last_len < blockSize - lenSize:
        pad[blockSize-lenSize:blockSize] = bytes.from_uint64_be(uint64(len_bits))
        h = compress(pad[0:blockSize],h)
    else:
        pad[(2*blockSize)-lenSize:2*blockSize] = bytes.from_uint64_be(uint64(len_bits))
        h = compress(pad[0:blockSize],h)
        h = compress(pad[blockSize:2*blockSize],h)
    result : bytes_t = bytes.from_uint32s_be(h)
