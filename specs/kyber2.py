...
variant_du_t  = refine(nat_t, lambda x: x == 10 or x == 11)
variant_dv_t  = refine(nat_t, lambda x: x == 3 or x == 4 or x == 5)
...
kyber_polycompressedbytes : FunctionType = lambda d: d * 32 #d*kyber_n /8
kyber_polybytes : int = 384  #12 * kyber_n / 8
kyber_symbytes : int = 32
kyber_polyveccompressedbytes : FunctionType = lambda kyber_k,kyber_du: kyber_k * kyber_polycompressedbytes(kyber_du)
kyber_polyvecbytes : FunctionType = lambda kyber_k: kyber_k * kyber_polybytes
kyber_indcpa_publickeybytes : FunctionType = lambda kyber_k: kyber_polyvecbytes(kyber_k) + kyber_symbytes
kyber_indcpa_secretkeybytes : FunctionType = kyber_polyvecbytes
kyber_indcpa_bytes : FunctionType = lambda kyber_k,kyber_du,kyber_dv: kyber_polyveccompressedbytes(kyber_k,kyber_du) + kyber_polycompressedbytes(kyber_dv)
...
@typechecked
def kyber_ciphertext_t(kyber_k:variant_k_t, kyber_du:variant_du_t, kyber_dv:variant_dv_t) -> bytes_t:
    return bytes_t(kyber_ciphertextbytes(kyber_k,kyber_du,kyber_dv))
@typechecked
def sharedkeybytes_t(kyber_sharedkeybytes:int) -> bytes_t:
    return bytes_t(kyber_sharedkeybytes)

@typechecked
def Kyber(kyber_k:variant_k_t,kyber_du:variant_du_t,kyber_dv:variant_dv_t,kyber_sharedkeybytes:int) \
    ...
    zeta : natmod_t     = zqelem(17)
    halfn_inv : natmod_t     = zqelem(3303)
    zeta_inv : natmod_t   = zqelem(1175)

    @typechecked
    def zqpoly_ntt(p:zqpoly_t) -> zqpoly_t:
        np: vector_t = vector.create(kyber_n, zqelem(0))
        for i in range(kyber_n//2):
            irev: int = int(uintn.reverse(uint8(i))) // 2
            for j in range(kyber_n//2):
                np[2*i] += (p[2*j] * (zeta ** ((2 * irev + 1) * j)))
                np[2*i+1] += (p[2*j+1] * (zeta ** ((2 * irev + 1) * j)))
        return np
    ...
    def zqpoly_invntt(p:zqpoly_t) -> zqpoly_t:
        ...
        for i in range(kyber_n//2):
            for j in range(kyber_n//2):
                jrev:int = int(uintn.reverse(uint8(j))) // 2
                np[2*i] += (p[2*j] * (zeta_inv ** (2 * i * jrev)))
                np[2*i+1] += (p[2*j+1] * (zeta_inv ** (2 * i * jrev)))
            np[2*i] *= halfn_inv * (zeta_inv ** i)
            np[2*i+1] *= halfn_inv * (zeta_inv ** i)
        return np

    @typechecked
    def zqpolyvec_ntt(p:zqpolyvec_t) -> zqpolyvec_t:
        np: vector_t = vector.create(kyber_k, zqelem(0))
        for i in range(kyber_k):
            np[i] = zqpoly_ntt(p[i])
        return np

    @typechecked
    def zqpolyvec_invntt(p:zqpolyvec_t) -> zqpolyvec_t:
        np: vector_t = vector.create(kyber_k, zqelem(0))
        for i in range(kyber_k):
            np[i] = zqpoly_invntt(p[i])
        return np
    
    @typechecked
    def zqpoly_modq(p:zqpoly_t) -> zqpoly_t:
        np:vector_t = vector.create(kyber_n, zqelem(0))
    ...
    @typechecked
    def zqpolyvec_modq(p:zqpolyvec_t) -> zqpolyvec_t:
        np:vector_t = vector.create(kyber_k, zqelem(0))
        for i in range(kyber_k):
            np[i] = zqpoly_modq(p[i])
        return np
    ...
    def pack_sk(sk:zqpolyvec_t) -> bytes_t(kyber_indcpa_secretkeybytes(kyber_k)):
        #encode_12(sk mod+ q)
        return bytes.concat_blocks(array.map(encode(12), zqpolyvec_ntt(sk)), bytes.empty())
    ...
    def unpack_sk(packedsk:bytes_t(kyber_indcpa_secretkeybytes(kyber_k))) -> zqpolyvec_t:
        ...
        res = zqpolyvec_invntt(array.map(decode(12), res))
        return res

    @typechecked
    def pack_pk(pk:zqpolyvec_t, seed:symbytes_t) -> bytes_t(kyber_indcpa_publickeybytes(kyber_k)):
        #(encode_12(pk mod+ q) || seed)
        return bytes.concat_blocks(array.map(encode(12), zqpolyvec_ntt(pk)), seed)
    ...
    def unpack_pk(packedpk:bytes_t(kyber_indcpa_publickeybytes(kyber_k))) -> tuple_t (zqpolyvec_t, symbytes_t):
        ...
        res : array_t(array_t)
        seed : array_t
        res, seed = bytes.split_blocks(packedpk, 384)
        pk : array_t = zqpolyvec_invntt(array.map(decode(12), res))
        return (pk, seed)

    @typechecked
    def pack_ciphertext(b:zqpolyvec_t, v:zqpoly_t) -> bytes_t(kyber_indcpa_bytes(kyber_k,kyber_du,kyber_dv)):
    ...
    @typechecked
    def unpack_ciphertext(c:bytes_t(kyber_indcpa_bytes(kyber_k,kyber_du,kyber_dv))) -> tuple_t (zqpolyvec_t, zqpoly_t):
        #(decompress_q(decode_du(c), du), decompress_q(decode_dv(c_v), dv))
        u1 : array_t(array_t)
        v1 : array_t
        u1, v1 = bytes.split_blocks(c, 32 * kyber_du)
    ...
    def genAij(seed:symbytes_t) -> FunctionType: # Callable[[int,int],zqpoly_t]:
        ...
        def _genAij(a:int, b:int) -> zqpoly_t:
            return zqpoly_invntt(genAij_hat(seed, uint8(a), uint8(b)))
        return _genAij
    ...
    def kyber_cpapke_encrypt(m:symbytes_t,
                             ...
                             coins:symbytes_t) -> \
                             bytes_t(kyber_indcpa_bytes(kyber_k,kyber_du,kyber_dv)):
        t : zqpolyvec_t
        rho : symbytes_t
    ...
    @typechecked
    def kyber_cpapke_decrypt(c:bytes_t(kyber_indcpa_bytes(kyber_k,kyber_du,kyber_dv)),
    ...
    def crypto_kem_enc(pk:kyber_publickey_t(kyber_k),msgcoins:symbytes_t) -> \
                       tuple_t (kyber_ciphertext_t(kyber_k,kyber_du,kyber_dv), sharedkeybytes_t(kyber_sharedkeybytes)):
        ...
        kr[kyber_symbytes:(2*kyber_symbytes)] = sha3_256(kyber_ciphertextbytes(kyber_k,kyber_du,kyber_dv), ct)
        ss : vlbytes_t = shake256(2*kyber_symbytes, kr, kyber_sharedkeybytes)
        return (ct, ss)

    @typechecked
    def crypto_kem_dec(ct:kyber_ciphertext_t(kyber_k,kyber_du,kyber_dv), sk:kyber_secretkey_t(kyber_k)) -> \
                       sharedkeybytes_t(kyber_sharedkeybytes):
        sk1 : bytes_t
        x : bytes_t
        pk : bytes_t
        sk2 : bytes_t
