...
let keccak_rotc : lseq_rotc_t_24_t = array (array_createL [1; 3; 6; 10; 15; 21; 28; 36; 45; 55; 2; 14; 27; 41; 56; 8; 25; 43; 62; 18; 39; 61; 20; 44])

let keccak_piln : lseq_pilns_t_24_t = array (array_createL [10; 7; 11; 17; 18; 3; 5; 16; 8; 21; 24; 4; 15; 23; 19; 13; 12; 2; 20; 14; 22; 9; 6; 1])

let keccak_rndc : lseq_rndc_t_24_t = array (array_createL [uint64 1; uint64 32898; uint64 9223372036854808714; uint64 9223372039002292224; uint64 32907; uint64 2147483649; uint64 9223372039002292353; uint64 9223372036854808585; uint64 138; uint64 136; uint64 2147516425; uint64 2147483658; uint64 2147516555; uint64 9223372036854775947; uint64 9223372036854808713; uint64 9223372036854808579; uint64 9223372036854808578; uint64 9223372036854775936; uint64 32778; uint64 9223372039002259466; uint64 9223372039002292353; uint64 9223372036854808704; uint64 2147483649; uint64 9223372039002292232])

let readLane (s : state_t) (x : index_t) (y : index_t) : uint64_t = 
s.[x +. (5 *. y)]
...
let rotl (a : uint64_t) (b : rotval_u64_t) : uint64_t = 
uintn_rotate_left a b

let state_theta_inner_C (s : state_t) (i : nat_t) (_C : lseq_uint64_5_t) : lseq_uint64_5_t = 
let _C = _C.[i] <- ((((readLane s i 0) ^. (readLane s i 1)) ^. (readLane s i 2)) ^. (readLane s i 3)) ^. (readLane s i 4) in 
_C
...
let state_pi_rho_inner (t : size_nat_24_t) (current : uint64_t) (s_pi_rho : state_t) : (uint64_t * state_t) = 
...
let (current,s_pi_rho) = repeati 24 (fun t (current,s_pi_rho) -> state_pi_rho_inner t current s_pi_rho) (current,s_pi_rho) in 
s_pi_rho

let state_chi_inner (s_pi_rho : state_t) (y : index_t) (x : index_t) (s : state_t) : state_t = 
let s_chi = writeLane s x y ((readLane s_pi_rho x y) ^. (~ (readLane s_pi_rho ((x +. 1) % 5) y) &. (readLane s_pi_rho ((x +. 2) % 5) y))) in 
s_chi
...
let absorb_last (delimitedSuffix : uint8_t) (rateInBytes : size_nat_0_200_t) (rem : nat_t) (input_b : lbytes_200_t) (inputByteLen : size_nat_t) (s : state_t) : state_t = 
...
let s = if (not (delimitedSuffix &. (uint8 128)) = (uint8 0)) && rem = (rateInBytes -. 1) then state_permute s else s in 
let s = absorb_next s rateInBytes in 
s

let absorb_inner (rateInBytes : size_nat_0_200_t) (block : lbytes_200_t) (s : state_t) (i : nat_t) : state_t = 
let s = loadState rateInBytes (array_slice block (i *. rateInBytes) ((i *. rateInBytes) +. rateInBytes)) s in 
let s = state_permute s in 
s

let absorb (s : state_t) (rateInBytes : size_nat_0_200_t) (inputByteLen : size_nat_t) (input_b : vlbytes_t) (delimitedSuffix : uint8_t) : state_t = 
...
let squeeze_inner (rateInBytes : size_nat_0_200_t) (outputByteLen : size_nat_t) (i : nat_t) (s : state_t) : (lbytes_200_t * state_t) = 
...
let block = array_create 200 (uint8 0) in 
let (block,output,s) = repeati outBlocks (fun i (block,output,s) -> let (block,s) = squeeze_inner rateInBytes outputByteLen i s in 
let output = array_update_slice output (i *. rateInBytes) ((i *. rateInBytes) +. rateInBytes) (block) in 
(block,output,s)) (block,output,s) in 
...
let keccak (rate : size_nat_t) (capacity : size_nat_t) (inputByteLen : size_nat_t) (input_b : vlbytes_t) (delimitedSuffix : uint8_t) (outputByteLen : size_nat_t) : vlbytes_t = 
