...
let working_state = repeati 10 (fun x working_state -> double_round working_state) working_state in 
let working_state = repeati 16 (fun i working_state -> working_state.[i] <- working_state.[i] +. st.[i]) working_state in 
working_state
...
let (blocks,ctr) = repeati (array_length blocks) (fun i (blocks,ctr) -> let blocks = blocks.[i] <- chacha_encrypt_block st ctr blocks.[i] in 
let ctr = ctr +. counter in 
(blocks,ctr)) (blocks,ctr) in 
let keyblock = chacha20_key_block0 key ctr nonce in 
let last_block = array_update_slice last_block 0 (array_length last) (last) in 
let last_block = xor_block last_block keyblock in 
let last = array_slice last_block 0 (array_length last) in 
...
let chacha20_encrypt_bytes (key : key_t) (counter : uint32_t) (nonce : nonce_t) (msg : vlbytes_t) : vlbytes_t = 
